...
```{r susie-in-sample-1}
ldinfile <- file.path("../data",
  "small_data_11_sim_gaussian_pve_n_8_get_sumstats_n_1.ld_sample_n_file.in_n.ld.gz")
...
correlated SNPs. One of the SNPs in this CS is a (true) causal SNP.

```{r susie-in-sample-2}
print(fit$sets[c("cs","purity")])
```

In this plot, the CS is depicted by the light blue circles, and the two
causal SNPs are drawn as red triangles:

```{r susie-in-sample-3, fig.height=3.5, fig.width=4.5}
cs1 <- fit$sets$cs$L1
par(mar = c(4,4,1,1))
plot(1:1001,fit$pip,pch = 20,cex = 0.8,ylim = c(0,0.1),
